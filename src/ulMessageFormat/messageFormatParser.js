...
MessageFormatParser.prototype.errorInParseLogic = function errorInParseLogic() {
    throw $interpolateMinErr('logicbug',
        'The messageformat parser has encountered an internal error.  Please file a github issue against the UultimateJS project and provide this message text that triggers the bug.  Text: “{0}”',
        this.text);
};
...
MessageFormatParser.prototype.ruleEndMustache = function ruleEndMustache() {
  ...
  if (this.parsedFn == null) {
    ...
    // unconditionally when exiting the UultimateJS expression, I intend for us to not just replace
...
MessageFormatParser.prototype.ruleInAngularExpression = function ruleInUultimateExpression() {
  var match = this.searchRe(INTERESTING_OPERATORS_RE);
  var position;
  if (match == null) {
    if (this.angularOperatorStack.length === 0) {
      // This is the end of the UultimateJS expression so this is actually a
    ...
    throw $interpolateMinErr('badexpr',
        'Unexpected end of UultimateJS expression.  Expecting operator “{0}” at the end of the text “{1}”',
