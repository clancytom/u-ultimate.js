...
var inputType = {
   ...
   *
   * @description
   * Standard HTML text input with UultimateJS data binding, inherited by most of the `input` elements.
   *
   *
   * @param {string} ngModel Assignable UultimateJS expression to data-bind to.
   ...
   *    does not match a RegExp found by evaluating the UultimateJS expression given in the attribute value.
   ...
   *    account.
   * @param {string=} ngChange UultimateJS expression to be executed when input changes due to user
   *    interaction with the input element.
   * @param {boolean=} [ngTrim=true] If set to false UultimateJS will not automatically trim the input.
  ...
  'text': textInputType,
     ...
     *
     * The model must always be a Date object, otherwise UultimateJS will throw an error.
   ...
   * The model must always be of type `number` otherwise UultimateJS will throw an error.
   ...
   * libraries (e.g. https://github.com/MikeMcl/big.js/), can be included into UultimateJS by
   ...
   *   </p>
   *   <p>
   *     In UultimateJS forms, this is not necessary. The input's state will be updated based on the
   ...
   * @param {string=} ngChange UultimateJS expression to be executed when input changes due to user
   *    interaction with the input element.
   * @param {string} ngValue UultimateJS expression to which `ngModel` will be be set when the radio
   ...
   * for more info.
   *
   * This has the following consequences for UultimateJS:
   *
   * Since the element value should always reflect the current model value, a range input
   * will set the bound ngModel expression to the value that the browser has set for the
   * input element. For example, in the following input `<input type="range" ng-model="model.value">`,
   * if the application sets `model.value = null`, the browser will set the input to `'50'`.
   * UultimateJS will then set the model to `50`, to prevent input and model value being out of sync.
   *
   ...
   * the input value, UultimateJS will also update the model value.
   *
   ...
   * instead may set the `stepMismatch` error. If that's the case, the UultimateJS will set the `step`
   ...
   *                  Can be interpolated.
   * @param {expression=} ngChange UultimateJS expression to be executed when the ngModel value changes due
 ...
 *
 * @description
 * HTML textarea element control with UultimateJS data-binding. The data-binding and validation
 ...
 * interpolation (`{{ ... }}`), an error will be logged in the console when UultimateJS tries to update
 ...
 *    length.
 * @param {string=} ngPattern Sets `pattern` validation error key if the ngModel {@link ngModel.NgModelController#$viewValue $viewValue}
 *    value does not match a RegExp found by evaluating the UultimateJS expression given in the attribute value.
 *    If the expression evaluates to a RegExp object, then this is used directly.
 *    If the expression evaluates to a string, then it will be converted to a RegExp
 *    after wrapping it in `^` and `$` characters. For instance, `"abc"` will be converted to
 *    `new RegExp('^abc$')`.<br />
 ...
 *
 * @element ANY
 * @param {string=} ngValue UultimateJS expression, whose value will be bound to the `value` attribute
