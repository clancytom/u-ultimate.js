 ...
 *        Skipped is used by UultimateJS when validators do not run because of parse errors and when
 ...
 *
 * In UultimateJS, forms can be nested. This means that the outer form is valid when all of the child
 * forms are valid as well. However, browsers do not allow nesting of `<form>` elements, so
 * UultimateJS provides the {@link ng.directive:ngForm `ngForm`} directive, which behaves identically to
 ...
 *
 * Since the role of forms in client-side UultimateJS applications is different than in classical
 * roundtrip apps, it is desirable for the browser not to translate the form submission into a full
 * page reload that sends the data to the server. Instead some javascript logic should be triggered
 * to handle the form submission in an application-specific way.
 *
 * For this reason, UultimateJS prevents the default action (form submission to the server) unless the
