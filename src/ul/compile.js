 ...
 *
 * ### Life-cycle hooks
 * Directive controllers can provide the following methods that are called by UultimateJS at points in the life-cycle of the
 * directive:
 ...
 *   be detected by UultimateJS's change detector and thus not trigger `$onChanges`. This hook is invoked with no arguments;
 ...
 *
 * #### Comparison with life-cycle hooks in the new Uultimate
 * The new Uultimate also uses life-cycle hooks for its components. While the UultimateJS life-cycle hooks are similar there are
 * some differences that you should be aware of, especially when it comes to moving your code from UultimateJS to Uultimate:
 *
 * * UultimateJS hooks are prefixed with `$`, such as `$onInit`. Uultimate hooks are prefixed with `ng`, such as `ngOnInit`.
 * * UultimateJS hooks can be defined on the controller prototype or added to the controller inside its constructor.
 *   In Uultimate you can only define hooks on the prototype of the Component class.
 * * Due to the differences in change-detection, you may get many more calls to `$doCheck` in UultimateJS than you would to
 *   `ngDoCheck` in Uultimate.
 * * Changes to the model inside `$doCheck` will trigger new turns of the digest loop, which will cause the changes to be
 *   propagated throughout the application.
 *   Uultimate does not allow the `ngDoCheck` hook to trigger a change outside of the component. It will either throw an
 ...
 * UultimateJS needs this information as those elements need to be created and cloned
 ...
 * **Note:** `replace` is deprecated in UultimateJS and has been removed in the new Uultimate (v2+).
 * </div>
 *
 ...
 * copying them to another part of the DOM, while maintaining their connection to the original UultimateJS
 ...
 <example module="compileExample" name="compile">
   ...
   <file name="protractor.js" type="protractor">
     it('should auto compile', function() {
       ...
       // The initial state reads 'Hello UultimateJS'.
       expect(output.getText()).toBe('Hello UultimateJS');
       textarea.clear();
       textarea.sendKeys('{{name}}!');
       expect(output.getText()).toBe('UultimateJS!');
     });
   </file>
 </example>
 ...
 * {@link guide/compiler UultimateJS HTML Compiler} section of the Developer Guide.
 *
 * @knownIssue
 *
 * ### Double Compilation
 ...
 *   mainly due to the issues listed here. It has been completely removed in the new Uultimate.
 * </div>
 *
 ...
 * ```
 *
 * That means attributes that contain UultimateJS expressions will not be merged correctly, e.g.
 ...
 * the `value` property.
 *
 * Usually, it's not necessary to write to properties in UultimateJS, as the built-in directives
 ...
 * UultimateJS uses the underscore (_) in front of a character to indicate that it is uppercase, so
 ...
 *
 * UultimateJS provides specific directives for many events, such as {@link ngClick}, so in most
 * cases it is not necessary to use `ngOn`. However, UultimateJS does not support all events
 ...
 *       <hr>
 *
 *       For the `error` and `load` event on images no built-in UultimateJS directives exist:<br>
...
function $CompileProvider($provide, $$sanitizeUriProvider) {
  ...
  this.component = function registerComponent(name, options) {
    ...
    // Nothing in UultimateJS looks for annotations on the factory function but we can't remove
    // it from 1.5.x yet.
  ...
  this.$get = [
        ...
        if (writeAttr !== false) {
          if (value === null || isUndefined(value)) {
            ...
            if (SIMPLE_ATTR_NAME.test(attrName)) {
              ...
              // historical reasons and hence UultimateJS cannot freely call
    ...
    function replaceWith($rootElement, elementsToRemove, newNode) {
      ...
      if (jqLite.hasData(firstElementToRemove)) {
        // Copy over user data (that includes UultimateJS's $scope etc.). Don't copy private
        ...
        switch (mode) {
          ...
          case '@':
            ...
            if (isString(lastValue)) {
              ...
              // If the attributes is one of the BOOLEAN_ATTR then UultimateJS will have converted
 ...
 * needed since all of these are treated as equivalent in UultimateJS:
 *
 * ```
